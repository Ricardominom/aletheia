...
function MetricCard({ value, unit, label, trend, sublabel, color = 'teal' }: MetricCardProps) {
  return (
    <div className="target-card p-3 animate-scale-in relative group">
      ...
      <div className="text-[10px] text-gray-300 uppercase mt-2 leading-tight group-hover:text-white transition-colors duration-300">
        {label}
        <div className="text-[9px] text-gray-400 normal-case mt-0.5 group-hover:text-gray-300 transition-colors duration-300">{sublabel}</div>
      </div>
    </div>
  );
}
    ...
    <div className="target-card p-3 animate-slide-up group">
      <div className="flex gap-3">
        <div className="relative">
          ...
          <img src={avatar} alt={username} className="relative w-8 h-8 rounded-full ring-1 ring-accent-teal/30" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-accent-teal group-hover:text-accent-teal/80 transition-colors duration-300">{username}</span>
            <span className="text-xs text-gray-500">{timestamp}</span>
          </div>
          <p className="text-xs text-gray-300 mt-1 line-clamp-2 group-hover:text-white transition-colors duration-300">{content}</p>
        </div>
      </div>
    </div>
  );
...
export default function SocialListening() {
  ...
  const data = {
    ...
    datasets: [
      {
        data: [400, 600, 300, 500, 400, 300, 250, 200, 400, 2000, 1000, 800, 600, 1000, 600, 500, 600, 500, 800, 1000, 1200, 1800, 1000, 800],
        borderColor: '#14B8A6',
  ...
  return (
    <div className="glassmorphic-container p-5 h-[480px] overflow-hidden animate-scale-in">
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        <MetricCard
          value={metrics.mentions.toString()}
          unit="K"
          label="MENCIONES"
          trend="up"
        ...
        />
        <MetricCard
          value={metrics.impressions.toString()}
          unit="M"
          label="IMPRESIONES"
          trend="up"
          sublabel="EN LAS ÚLTIMAS 24 HRS"
